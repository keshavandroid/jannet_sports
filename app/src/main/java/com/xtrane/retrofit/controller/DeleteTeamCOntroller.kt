...
class DeleteTeamCOntroller(var context: Activity, internal var view: IDeleteTeamView):IDeleteTeamController {
    override fun callDeleteTeamApi(id: String, token: String, teamId: String) {
        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.deleteTeam(id,token,teamId)
                view.showLoader()
