...
class AcceptRejectRequestController(var context: Activity, internal var view: IAcceptRejectEventView):IAcceptRejectEventController {

    override fun acceptRejectEvent(id: String, token: String,childId: String,eventId: String, status: String) {
        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.acceptRejectEvent(id,token,childId,eventId,status)
            ...
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    if (response.getStatus() == 1) {
                        ...
                        view.onAcceptRejectEventSuccess()
    ...
    override fun acceptRejectTicket(id: String, token: String,childId: String,eventId: String, status: String, matchID : String) {
        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.acceptRejectTicket(id,token,childId,eventId,status,matchID)
        ...
        RetrofitHelper.callApi(call, object : RetrofitHelper.ConnectionCallBack{
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    if (response.getStatus() == 1) {
                        //var data = response.getResult()
                        view.onAcceptRejectTicketSuccess()
