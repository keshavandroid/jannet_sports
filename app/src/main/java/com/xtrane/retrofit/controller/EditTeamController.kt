...
class EditTeamController(var context: Activity, var registerControllerInterface: RegisterControllerInterface) {



    fun EditTeam(registerData: RegisterData) {
...
//        SharedPrefUserData(context).getSavedData().id
        ...
        var teamId: RequestBody =
            RequestBody.create("text/plain".toMediaTypeOrNull()!!, registerData.teamId)
        ...
        val call: Call<ResponseBody?>? = RetrofitHelper.getAPI()
            .editTeam(id,token,teamId,teamName,description,image)
