...
import com.e.jannet_stable_code.retrofit.alreadyjointeam.AlreadyJoinTeamBaseResponse
import com.e.jannet_stable_code.retrofit.coachlogindata.CoachLoginBaseResponse
import com.e.jannet_stable_code.viewinterface.ICoachAlreadyJoinTeamView
...
class CoachAlreadyJoinTeamController(var context: Activity, internal var view: ICoachAlreadyJoinTeamView):ICoachAlreadyJoinTeamController {
    override fun callCoachJoinTeamApi(id: String, token: String) {

        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.getJoinTeamList(id,token)
        ...
        RetrofitHelper.callApi(call, object : RetrofitHelper.ConnectionCallBack{
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    val response = gson.fromJson(reader, AlreadyJoinTeamBaseResponse::class.java)

                    if (response.getStatus() == 1) {
                        var data = response.getResult()
                        view.alredyJoinTeamSuccess(data!!)
