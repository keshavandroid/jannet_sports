...
private const val TAG = "CheckPhEmailOtpCont"
class CheckPhEmailOtpController(val context: Activity,val email:String,val phno:String, val userId:String, internal val controllerInterface: ControllerInterface) {
    init {
        callApi()
    }
    private fun callApi() {
        ...
        val call: Call<ResponseBody?>? = RetrofitHelper.getAPI().checkPhoneEmailOtpApi(email,phno,userId)
            ...
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    if (response.getStatus() == 1) {
                        SharedPrefUserData(context).setRegisterStep("2")
