...
class BookingListController(var context: Activity, internal var view: IBokingListView):IBookingListController  {
    override fun callBookingDetailApi(id: String, token: String, event_id: String) {
        view.showLoader()

        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.getBookingList(id,token,event_id)
        ...
        RetrofitHelper.callApi(call, object : RetrofitHelper.ConnectionCallBack{
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    val response = gson.fromJson(reader, BookingListBaseResponse::class.java)

                    if (response.getStatus() == 1) {

                        val data = response.getResult()
                        view.onBookingDetailSuccess(data)
