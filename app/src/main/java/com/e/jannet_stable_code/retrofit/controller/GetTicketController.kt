...
import com.e.jannet_stable_code.retrofit.gettickitsdata.GetTicketsBaseResponse
...
class GetTicketController(var context: Activity, internal var view: IGetTicketsView):IGetTicketsCOntroller {
    override fun callGetTicketsApi(id: String, token: String, event_id: String) {
        view.showLoader()

        val apiInterface: UserServices = APIClient.getClient()!!.create(UserServices::class.java)
        val call: Call<ResponseBody?>? = apiInterface.getTickets(id,token,event_id)
        ...
        RetrofitHelper.callApi(call, object : RetrofitHelper.ConnectionCallBack{
            override fun onSuccess(body: Response<ResponseBody?>?) {
                ...
                try {
                    ...
                    val response = gson.fromJson(reader, GetTicketsBaseResponse::class.java)

                    if (response.getStatus() == 1) {

                        val data = response.getResult()
                        view.onGetTicktetsSuccess(data)
